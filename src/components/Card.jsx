export default function Component({ el }) {
  return (
    <div data-testid="card" className="card w-25 m-4 d-flex flex-col gap-2">
      <img
        data-testid={`img-recipe-${el.id}`}
        src={el.image}
        className="card-img-top"
        alt={el.name}
      />
      <h5 data-testid={`title-recipe-${el.id}`} className="px-3 card-title">
        {el.name}
      </h5>
      <div className="px-3 d-flex align-items-center gap-2">
        ...
        <p data-testid={`rating-recipe-${el.id}`} className="m-0">
          {el.rating}
        </p>
      </div>
      ...
      <a
        data-testid={`link-recipe-${el.id}`}
        href={`https://dummyjson.com/recipes/${el.id}`}
        className="px-3 pb-2 text-warning"
      >
        READ MORE
      </a>
    </div>
